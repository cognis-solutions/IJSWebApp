...
export class JoSummaryTableComponent implements OnInit {
  ...
  @Output() createNextJobOrder: EventEmitter<any> = new EventEmitter();
  

  constructor(private _user:UserTypeService,private _spinner: SpinnerServiceService, private _processjoSearchService: ProcessjoSearchService, public _serverErrorCode: ServerErrorcodeService, private _http: Http,private _sessionTimeOutService:SessionTimeOutService) { }
  ...
  getPrcessJoBackEndData() {
    ...
    // if(this._user["appUserType"]=="Global")
    // {
    //   var remark =this.joSummerySaveObj.lstJOSummaryParam.remark;
    //   console.log(remark);
    //   UIkit.modal('#HQ_User-warning-modal').show();
    // }
    ...
    routeBackEndData.subscribe(
      ...
      if(data == "userSessionExpired"){
        ...
        data.result.forEach(element =>{
          if(data.result['0'].joSummery.hasBundle>0)
          //if(true)
        {   
               UIkit.modal('#requiredFieldsQuantity-warning-modal').show();
        }
      });
 ...
 $('#create-adhoc-jo-modal').addClass('uk-open').hide();
    ...
    this.joSummeryTableData.forEach(element =>{
      let tempobj = {
        ...
        'paymentFSC': element.joSummery.paymentFSC,
         'quantity' : element['jobOrders'][0].quantity,
         'checked'  :this.joSummerySaveObj.lstJOSummaryParam['0'].checked,
         'status' : this.joSummerySaveObj.lstJOSummaryParam.status,
         'remark': this.joSummerySaveObj.lstJOSummaryParam.remark
      }
    ...
    routeBackEndData.subscribe(
      (data) => {
        if(data == "userSessionExpired"){
          ...
          $("#successfull-Job-Order-modal-sections").addClass('uk-open').show();      
  ...
  nextSummary(e)
  {
    ...
    UIkit.modal('#successfull-Job-Order-modal-sections').hide();
    this.createNextJobOrder.emit(true);
  ...
  changeFind(e) {
    //UIkit.modal('#successfull-Job-Order-modal-sections-Adhoc').hide();
    UIkit.modal('#successfull-Job-Order-modal-sections').hide();
  ...
  resetJoSummeryTable(e) {
   // debugger;
   // this.joSummerySaveObj=[];
    $("#successfull-Job-Order-modal-sections").addClass('uk-open').hide();
    $("#booking-bl-process-jo").addClass('uk-open').hide();
    let tempObj = {
  ...
  calculateDate(tableRow){
    //debugger;
    let rate=tableRow.joSummery.amount;
    let row:any =tableRow['jobOrders'];
    if(row!= null){
      rate=0;
      for(let i=0;i<row.length;i++){
        let obj:any=row[i];
        rate=rate+parseFloat(obj.rate)*parseFloat(obj.quantity);
      }

    }
    tableRow.joSummery.amount=rate;
    return rate;
  }

  calculateRate(row){
    return parseFloat(row.rate)*parseFloat(row.quantity)
  }
}
