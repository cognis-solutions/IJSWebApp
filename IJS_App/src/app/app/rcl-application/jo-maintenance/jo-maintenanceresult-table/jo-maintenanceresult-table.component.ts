...
//import { UserTypeService}   from "app/user/user-type.service";
declare var UIkit: any;
...
export class JoMaintenanceresultTableComponent implements OnInit {
  ...
  constructor(private _userTypeService : UserTypeService,private _maintenanceService: JoMaintenanceSearchService, private _serverErrorCode: ServerErrorcodeService, private _spinner: SpinnerServiceService, private _sortTable: JoMaintenanceSortingService, private _rclappUrlService: RclappUrlService, private winRef: WindowRefService
  ,private spinner: SpinnerServiceService,modalService: ModalDialogService, viewRef: ViewContainerRef,private clService:ContainerListService,private _sessionTimeOutService:SessionTimeOutService,private _userService:UserService,private _lookupData: LookUpdataServiceService) {
  ...
  jobOrderCancelText = "";
  @Output() emptyContainerListArray = new EventEmitter();
  ...
  ngOnInit() {
   //alert(this._userService['appUserType'])
   // console.log("user type :"+ this._userTypeService['appUserType'] ) ;  
    //console.log("detailType :"  +this.tableDataMaintainJoSearch[0].detailType) ;  
    this.getUserData();
   
    //console.log(this.getUserData()+"useerrr");
    //console.log(this.maintainJoSearchData.maintainJoParam['userData']);
    ...
    this.columnDefs2 = [
      {
        ...
        suppressMenu: true, hide: this.colhide,
         cellStyle: {
          'padding-top': '2px',
        }
      },
...
return false
  ...
  d: any;
  myDiveColorChange(row,id){
   
    this.d=id;
   // debugger;
   //console.log("MyFuction out side if   = " + row.contDetailJO.length);
  
   if(row.contDetailJO.length!=0 && row.contDetailJO.length!=null)
   {
   for(var i=0;i<row.contDetailJO.length;i++)
   {
   ...
   // debugger;
   if(row.contDetailJO[i].eqNumber!=null){
     if( row.contDetailJO[i].eqNumber.startsWith("20")||row.contDetailJO[i].eqNumber.startsWith("40")||
     row.contDetailJO[i].eqNumber.startsWith("45")|| (row.contDetailJO[i].contSize=="" || row.contDetailJO[i].contType=="" )
                                  ||(row.contDetailJO[i].amountUSD=="" && row.contDetailJO[i].amount=="" && row.contDetailJO[i].amountLumpsum=="" &&row.contDetailJO[i].amountBkgBl=="" ))
      { 
      document.getElementById(this.d+"_"+this.d).classList.add("x");
      break;
      }
    }
   }
  ...
  //for AddEquipment--nikash//
  equipmentBrowserModal(e,row,index) 
  {
	  this.selectedGridApi=this.gridApi[index]
 
   	this.selectedRow = row;
  
    this._joInquiryEquipmentBrowser.openEmptyEquipmentsModal(row);
  }
 ...
 @Output() searchDataJoInquiry: EventEmitter<any> = new EventEmitter();
  ...
  remarkjson=[];
  //#NIIT CR6 >>>>BEGIN
  approveJobOrderList(e,row){
     //debugger;
     this.getLocLookUpData('getJOLog','','',this.tempRowObj["JoNumber"],'');
     console.log(this.remarkjson);
     
     
    //if selected rows conatins lumpsum the ask for confirmation 
   // sid comment 
         //sid  if(this.containsLumpSum){
         //sid   $('#approve-job-order').addClass('uk-open').show();
         //sid   }
   //if selected rows doesn't contains lumpsum then approve job orders directly     
        //sid   else{
        //sid    this.approveJobOrder('');
        //sid  }
    // sid comment end
  }
  ...
  getLocLookUpData(lookupTpye, type, eve, inpuvaluevalue, wildCard) {     
    ...
    backendData.subscribe(
      (data) => {
        this.remarkjson=data;
        this._spinner.hideSpinner();
        this.approvePopUpForRemark();
      },
      (err) => {      
        this._spinner.hideSpinner();
        // A client-side or network error occurred. Handle it accordingly.
        this.remarkjson=null; 
        this.approvePopUp(); 
      }
    )
   }

   approvePopUpForRemark()
   {
    // debugger;
     var count=0;
     if(this.remarkjson!=null)
     {
       for(var i=0;i<this.remarkjson.length;i++)
       {
             if(this.remarkjson[i]["Activity"]=="The JobOrder is created with default Vendor but th")
             {
            ...
            if(count==0)
            {
              $('#approve-job-order-priority').addClass('uk-open').show();
            }
            else if(count>0)
            {
              this.approvePopUp();
  ...
  approveJobOrder(e) {
   // debugger;
    console.log(this._rclJoLog.remarkjson); 
  ...
  public joNewCntrList:any[]=new Array();
  public joNewCntrListForSave:any[]=new Array();

  saveJobOrder(e, rowObj) {
    //debugger;
	for(var i=0;i<this.joNewCntrList.length;i++)
	{
		if(this.joNewCntrList[i]["selected"])
		{
			this.joNewCntrListForSave.push(this.joNewCntrList[i]);
		}
	}   
    let saveJobOrderSearchData = {
      joSaveListAll: {
        ...
        "joNewCntrList":this.joNewCntrListForSave,
        "rowDataObj":this.tempRowObj
    ...
    backendData.subscribe(
      (data) => {
        ...
        this.checkedSelectedRows = [];
		this.joNewCntrListForSave=new Array();
      },
      (err) => {
        this._spinner.hideSpinner();
      }
  ...
  private insertLegshowRouteList(e) {
    ...
    if (this.checkedSelectedRows.length == 1) {
      ...
      this.routeListBarge=this.checkedSelectedRows[0].barge;
      ...
      this._maintainjoRouteList.bargeValue = this.routeListBarge;
  ...
  private onRowSelected(e, row) {
	  ...
	  if(this.joNewCntrList.length>0){
		row.contDetailJO[e.node.id].selected=e.node.selected;  
	  }	
  ...
  tempAdhoc:any;
  res:any;
  updateEqDetailsAdd(e) {
   //alert("updateEqDetailsAdd");
    this.bookingBlNoTemp="";
    if(this.selectedRow.jobOrdType!='' && this.selectedRow.jobOrdType!=null && this.selectedRow.jobOrdType!=undefined){
    this.tempAdhoc="";
    this.tempAdhoc=this.selectedRow.jobOrdType;
    this.res = this.tempAdhoc.split(" ");
    this.tempAdhoc=this.res[1];
    }
    if(this.selectedRow.contDetailJO.length>0)
      {
      this.bookingBlNoTemp=this.selectedRow.contDetailJO[0]["BkgOrBLNo"];
      }
    if(e.eqListSource != null){
      //console.log(e);
     // debugger;
      for(let i=0;i<e.length;i++){
        let row={ "equiptState": "No Status", "eqNumber": e[i].eqpNum, "oldContainerNo":  "", "BkgOrBLNo": this.bookingBlNoTemp, "contSize":  e[i].size, "contPercent": "0", "contType": e[i].type, "contEmptyOrLaden": this.tempAdhoc, "SOCorCOC": "COC", "currency": "", "amount": "", "amountUSD": "","contWeight": "", "startDate": "", "endDate": "", "amountBkgBl": "", "amountLumpsum": "" ,"JoNumber":this.selectedRow.JoNumber,
        "contractId":this.selectedRow.contractId,"vendorID":this.selectedRow.vendorID,selected:true}
       this.joNewCntrList.push(row);
      //console.log(this.joNewCntrList)
      this.selectedRow.contDetailJO.push(row)
      
      }
      
 
     this.selectedGridApi.setRowData(this.selectedRow.contDetailJO);

     this.selectedGridApi.forEachNode(function(node) {
    ...
    //this.emptyContainerListArray.emit(e);
   };

  joReplaceCntrList: any = [];
  updateEqDetails(e) {
  //alert('updateEqDetails');
  //debugger;
if(this.selectedRow==undefined){
    if (this.selectedRowData) {
      if(this.replaceObject != undefined){
        ...
        "contSize": parseInt(this.replaceObject.cntSize),
       ...
       if(this.joReplaceCntrList.length ==0){
         ...
         this.selectedRowData.data['selected']=true;
       }else{
     ...
     // this.selectedRowData.data['checked']=true;
       this.joReplaceCntrList.push(tempObj);
       this.selectedRowData.data['eqNumber'] = e[0].eqpNum;
       this.selectedRowData.data['selected']=true;
      
      } 
      //this.selectedRowData.rowModel.rootNode.selected=true;
     // debugger;
    //
    this.selectedRowData.gridOptionsWrapper.gridOptions.api.setRowData(this.selectedRowData.gridOptionsWrapper.gridOptions.rowData);
  
   this.selectedRowData.gridOptionsWrapper.gridOptions.api.forEachNode(function(node) {
...
function sizeTypeCostRenderer(params){
  ...
  if(params.data.amount == undefined){
    //document.getElementById("xxx").classList.add("x");
    params.data.amount = "";
  } else {
